...
## API 경로 (**/auth/naver 사용 권장**)

- **시작**: `GET /auth/naver/start?user_id=...&draft_id=...&user_name=...`
- **콜백**: `GET /auth/naver/callback` (네이버 redirect)

`/auth` 구간은 미들웨어에서 **로그인 리다이렉트 제외**되어, OAuth 링크 접속 시 로그인 화면으로 넘어가지 않습니다.  
`/api/naver/oauth/*` 는 미들웨어·프록시 이슈로 **로그인 리다이렉트가 발생할 수 있으므로 사용하지 마세요.**
...
| `NAVER_REDIRECT_URI` | 콜백 URL (네이버 앱에 등록된 값과 동일) | `https://medifirstall.vercel.app/auth/naver/callback` |
| `OAUTH_STATE_SECRET` | state HMAC용 비밀 (kakkaobot과 동일 권장) | 영문/숫자 임의 문자열 |

- `NAVER_REDIRECT_URI` 없으면 `VERCEL_URL`로 `https://${VERCEL_URL}/auth/naver/callback` 사용.
- **네이버 개발자센터** 앱의 Callback URL에 `https://medifirstall.vercel.app/auth/naver/callback` 등록 필수.
...
- `NAVER_OAUTH_START_PATH`: **`/auth/naver/start`** (기본값; `/api/naver/oauth/start` 사용 금지)
→ `!질문` 연동 링크가 위 API로 연결됩니다.

## 적용 순서 (OAuth 링크 → 로그인 화면 리다이렉트 해결)

1. **Vercel**  
   - `NAVER_REDIRECT_URI` = `https://medifirstall.vercel.app/auth/naver/callback`  
   - (그 외 NAVER_* / NAVER_OAUTH_SUPABASE_* / OAUTH_STATE_SECRET 설정 확인)

2. **네이버 개발자센터**  
   - 앱 **Callback URL**에 `https://medifirstall.vercel.app/auth/naver/callback` 추가

3. **kakkaobot**  
   - `NAVER_OAUTH_BASE_URL` = `https://medifirstall.vercel.app`  
   - `NAVER_OAUTH_START_PATH` = **`/auth/naver/start`** 또는 **미설정** (`.env`·`ecosystem`에 `/api/naver/oauth/start` 없도록)  
   - PM2 재시작: `pm2 restart kakkaobot-server` (또는 사용 중인 앱명)

4. **Vercel**  
   - medifirst 리포 push 후 배포 완료 대기  
   - `!질문` → 연동 링크가 `https://medifirstall.vercel.app/auth/naver/start?...` 로 안내되는지 확인
